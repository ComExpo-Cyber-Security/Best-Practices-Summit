...
/* ======= Countdown ========= */
let target_date = new Date("Jan 24, 2026").getTime();
...
function renderAttendees(list, container, countEl) {
  ...
  if (!Array.isArray(list) || list.length === 0) {
    container.innerHTML = '<div class="text-center py-4 text-muted">No attendees yet.</div>';
  ...
  if (list.length > previewCount) {
    ...
    moreEl.className = 'attendee-more d-flex align-items-center justify-content-center rounded-circle bg-danger text-white fw-bold';
...
function createAttendeeCard(att) {
  ...
  img.onerror = function () { this.src = 'assets/images/speakers/speaker-1.jpg'; };
  ...
  if (att.linkedin) {
    ...
    link.innerHTML = '<i class="fab fa-linkedin-in fa-lg" aria-hidden="true"></i>';
